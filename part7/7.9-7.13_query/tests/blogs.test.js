...
describe("POST /api/blogs", async () => {
  test("valid blog", async () => {
    ...
    const newBlogsResponse = await api
      .get("/api/blogs")
      .expect(200)
      .expect("Content-Type", /application\/json/);
  ...
  test("add missing likes property", async () => {
    ...
    const insertedBlogResponse = await api
      .post("/api/blogs")
      .set("Authorization", `Bearer ${authResponse.body.data.token}`)
      .send(newBlog)
      .expect(201)
    ...
    const includesLikesProperty = Object.keys(insertedBlog.data).includes(
      "likes",
    );
...
describe("DELETE /api/blogs", async () => {
  test("valid id", async () => {
    ...
    const blogDeletionResponse = await api
    ...
    assert.strictEqual(
      blogDeletionResponse.body.message,
    ...
    assert.strictEqual(
      newBlogsResponse.body.length,
      blogsResponse.body.length - 1,
    );
  });
  ...
  test("404 for non existing entry", async () => {
    ...
    const secondBlogDeletionResponse = await api
      .delete(`/api/blogs/${newBlogId}`)
      .set("Authorization", `Bearer: ${authResponse.body.data.token}`)
      .expect(404)
      .expect("Content-Type", /application\/json/);
    ...
    assert.strictEqual(
      secondBlogDeletionResponse.body.message,
      "Blog was not found.",
    );
  });
});
...
describe("PUT /api/blogs", async () => {
  ...
  test("update non-existing entry", async () => {
    ...
    const blogUpdateResponse = await api
      .put(`/api/blogs/${newBlogId}`)
      .set("Authorization", `Bearer: ${authResponse.body.data.token}`)
      .send({ likes: 1234 })
      .expect(404)
